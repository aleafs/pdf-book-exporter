...
def build_tree(directory: str, include_drafts: bool = False, parse_front_matter=None, should_include=None) -> Optional[
    Node]:
    index_path = None
...
def write_hierarchical_content(tmp, node: Node, book_dir: str,
                               temp_dir: str, temp_pngs: list, render: Config,
                               level: int = 1, cache_dir: str = None, process_images_in_content=None,
                               adjust_heading_levels_func=None) -> None:
    ...
    with open(node.path, "r", encoding="utf-8") as f:
        ...
        tmp.write(render.replace(content) + "\n\n")
    for child in node.children:
        write_hierarchical_content(tmp, child, book_dir, temp_dir, temp_pngs, render,
                                   level + 1, cache_dir,
                                   process_images_in_content, adjust_heading_levels_func)
    if level == 1:
        tmp.write('\\newpage\n\n')
